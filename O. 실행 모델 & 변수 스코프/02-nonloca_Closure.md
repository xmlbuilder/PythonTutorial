...
- nonlocal을 사용하면 내부 함수에서 외부 함수의 지역 변수 e를 수정 가능
...
- 따라서 inner()에서 e를 수정하려면 global이 아니라 nonlocal을 써야 함
...
| 상황 또는 목적             | 설명                                                                 |
|----------------------------|----------------------------------------------------------------------|
| 상태 저장 함수 구현         | 객체 없이도 함수 내부에서 상태를 유지하며 누적하거나 제어 가능               |
| 데코레이터 패턴 구현        | 외부 함수의 변수에 접근해 동작을 제어하는 데 클로저가 필수적임               |
| 캡슐화 및 은닉              | 외부에서 직접 접근할 수 없는 상태를 함수 내부에 숨겨 보안성과 안정성 향상     |
| 고차 함수 설계              | 동적으로 생성된 함수들이 외부 컨텍스트를 기억하고 활용할 수 있음              |
| 이벤트 핸들러 및 콜백 처리   | 이벤트 발생 시 내부 함수가 외부 상태를 기억하고 반응 가능                     |
...
| 항목             | 일반 함수                          | 클로저 함수                          | 비고                          |
|------------------|-------------------------------------|--------------------------------------|-------------------------------|
| 외부 변수 접근     | 불가능                              | 가능 (`nonlocal` 사용 시)             | 클로저는 외부 환경을 기억함     |
| 상태 유지         | 불가능                              | 가능 (호출 시마다 상태 누적 가능)     | 객체 없이도 상태 저장 가능      |
| `nonlocal` 사용   | 필요 없음                           | 필요함 (외부 지역 변수 수정 시)       | 내부 함수에서 외부 변수 수정 시 |
| 캡슐화 및 은닉     | 어려움                              | 가능 (외부에서 직접 접근 불가)        | 보안성과 테스트 용이성 향상     |
| 대표 활용 예시     | 일반 로직 처리                      | 데코레이터, 이벤트 핸들러, 고차 함수   | 함수형 설계에 적합              |

### 📊 Mermaid 다이어그램
```mermaid
flowchart TD
    A[outer 함수] --> B[count = 0]
    A --> C[inner 함수 정의]
    C --> D[nonlocal count]
    D --> E[count += 1]
    A --> F[return inner]
    F --> G[counter = outer()]
    G --> H[counter() 호출 시 inner 실행]
```
클로저는 단순한 문법이 아니라, 함수 중심 설계와 상태 캡슐화의 핵심 도구.
