...
pip install pytest
```
- pytest는 Python에서 가장 널리 쓰이는 테스트 프레임워크. 
- 간결한 문법과 강력한 기능 덕분에 실무에서도 많이 사용.
...
- 모든 테스트가 성공적으로 통과했음을 보여줌. . 하나당 하나의 테스트가 성공한 걸 의미함.
...
| CI/CD 연동         | GitHub Actions, Jenkins 등과 연동해 코드 푸시 시 자동 테스트 수행 가능             |
