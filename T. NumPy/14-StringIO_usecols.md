...
```
[(_, int) for _ in "abc"]] -> [('a', int), ('b', int), ('c', int)]
```
...
## 💬 한마디
- np.genfromtxt()는 단순한 텍스트 파싱을 넘어서 구조화된 데이터셋을 빠르게 만들 수 있는 강력한 도구.  
- 특히 usecols, names, dtype 조합은   실무에서 CSV 전처리, 로그 분석, 피처 선택에 자주 쓰임.
...
- 열 이름으로 접근 가능:
- 이렇게 만들면 numpy는 내부적으로 다음과 같이 저장해:
...
## 💬 한마디
- 구조화 배열은 단순한 숫자 배열을 넘어서 열 이름 기반의 데이터 처리를 가능하게 해줌.
- 특히 dtype=[(_, int) for _ in "abc"]는 반복문 없이 깔끔하게 열 이름과 타입을 지정하는 똑똑한 방식.
