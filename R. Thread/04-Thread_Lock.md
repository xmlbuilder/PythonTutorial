...
- GIL은 메모리/객체 내부 일관성을 지켜줌 (참조 카운트, 객체 구조 깨짐 방지).
- 하지만 **여러 단계로 이뤄진 연산을 한 덩어리로** 보장해주진 않음.
- 게다가 CPython은 일정 바이트코드 수/타이밍에서 스레드 전환이 일어나므로 위 같은 interleaving이 가능.
...
- 파일 읽기, 네트워크 요청, DB 접근처럼 대기 시간이 많은 작업에서는 스레드가 GIL을 잠깐 놓고 다른 스레드가 실행됨.
- 결과적으로 동시성 효과가 충분히 있음.
  - ✅ 예: 웹 크롤링, API 호출, 로그 수집, 파일 다운로드

### 2. GUI 응답 유지에 필수
- GUI 앱에서 백그라운드 작업을 threading으로 처리하면 메인 UI가 멈추지 않고 사용자 입력에 반응할 수 있음.
    - ✅ 예: PyQt, Tkinter, Kivy 기반 앱

### 3. 경량 작업 병렬 처리
- GIL이 있다고 해도, 작업이 가볍고 빠르면 스레드가 더 효율적일 수 있음.
- multiprocessing은 프로세스 생성 비용이 크기 때문에 작은 작업에는 오히려 느릴 수 있음.
